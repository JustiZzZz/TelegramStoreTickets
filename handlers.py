...
@router.message(F.from_user.id.in_(ADMIN_ID), F.reply_to_message, ~F.text.startswith('https://t.me/+'))
async def admin_reply_to_question(message: Message, bot: Bot):
    if "Текст вопроса:" in message.reply_to_message.text:
        ...
        if match:
            ...
            await bot.send_message(
                user_id,
                f"✉️ <b>Вам пришел ответ от администратора:</b>\n«{message.text}»",
                parse_mode="HTML"
            )
            ...
            for admin in ADMIN_ID:
                if admin != message.from_user.id:
                    await bot.send_message(
                        chat_id=admin,
                        text=(
                            f"ℹ️ <b>Администратор @{message.from_user.username} ответил на вопрос</b>\n"
...
async def notify_other_admins(bot: Bot, sender_id: int, user_id: int, action: str):
    sender_tag = f"@{(await bot.get_chat(sender_id)).username}" if (await bot.get_chat(sender_id)).username else f"ID: {sender_id}"
    for admin_id in ADMIN_ID:
        if admin_id != sender_id:
            await bot.send_message(
                chat_id=admin_id,
                text=(
                    f"📬 <b>Администратор {sender_tag} отправил {action}</b>\n"
                    f"Пользователю с ID: <code>{user_id}</code>"
                ),
                parse_mode="HTML"
            )
...
async def admin_approve_with_link(message: Message, bot: Bot):
    if message.reply_to_message.caption:
        ...
        if match:
            ...
            await bot.send_message(
                chat_id=user_id,
                text=SUCCESS_PAYMENT_MESSAGE.format(link=invite_link)
            )
            ...
            for admin in ADMIN_ID:
                if admin != message.from_user.id:
                    await bot.send_message(
                        chat_id=admin,
                        text=(
                            f"💳 <b>Администратор @{message.from_user.username} подтвердил оплату</b>\n"
                            f"👤 Пользователь: <code>{user_id}</code>\n\n"
                            f"🔗 Отправленная ссылка:\n{invite_link}"
                        ),
                        parse_mode="HTML"
                    )
